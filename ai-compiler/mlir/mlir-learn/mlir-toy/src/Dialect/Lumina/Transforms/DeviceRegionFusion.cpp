    ...
    for (auto op : ops) {
        ...
        for (auto type : op->getOperandTypes()) {
            if (auto shaped = llvm::dyn_cast_or_null<ShapedType>(type)) {
                for (auto idx : llvm::index_range(0, shaped.getRank())) {
                    if (shaped.isDynamicDim(idx)) {
                        name.append("d_");
                    } else {
                        name.append(llvm::to_string(shaped.getDimSize(idx)));
                        name.append("_");
                    }
                }
            }
...
struct DeviceRegionFusionPass
    ...
    using ::mlir::lumina::impl::DeviceRegionFusionPassBase<
