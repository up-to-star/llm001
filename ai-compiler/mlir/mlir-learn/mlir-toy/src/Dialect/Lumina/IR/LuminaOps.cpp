...
::llvm::LogicalResult LuminaSoftmaxOp::applyDataParallelism(
  ...
  if (!supportDataParallelism()) return llvm::failure();
...
static inline int getDeviceid(mlir::ArrayRef<::mlir::Operation *> ops) {
    if (auto tensor = llvm::dyn_cast_or_null<lumina::LMTensorType>(
...
llvm::LogicalResult LuminaDeviceKernelOp::verify() { return llvm::success(); }

llvm::LogicalResult LuminaDeviceKernelOp::FusionOps(
  ...
  auto kernel = rewriter.create<LuminaDeviceKernelOp>(loc, outputs_type, name,
  ...
  rewriter.create<LuminaReturnOp>(loc, output_val);
